...
@pytest.mark.parametrize("parent_identifiers, options", [
    (["root"], {}),
    (["foo", "bar"], {}),
    (["root"], {"weight": 5}),
    (["foo", "bar"], {"weight": 42})
    ...
    "with-parents",
    "with-weight",
    "with-parents-and-weight",
])
def test_graph_update_from_requirement_existing(
    mocker, mocked_resolver, mocked_package_extract, mocked_queue,
    parent_identifiers, options
):
    ...
    graph._update_from_requirement(
        requirement, parent_identifiers, mocked_queue, **options
    )

    graph._create_node_from_package.assert_not_called()

    assert graph.create_link.call_count == len(parent_identifiers)
    for parent in parent_identifiers:
        graph.create_link.assert_any_call(
            "_A==0.1.0", parent, requirement,
            weight=options.get("weight", 1)
        )

    node.add_parents.assert_called_once_with(parent_identifiers)
...
def test_graph_update_from_requirement_multi_packages(
    ...
    assert graph.create_link.call_count == 3 * len(parent_identifiers)
    for node in nodes:
        for parent in parent_identifiers:
            graph.create_link.assert_any_call(
                node.identifier,
